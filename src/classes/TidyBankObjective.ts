...
export class TidyBankObjective extends Objective {
  ...
  rawOreList = ['copper_ore', 'iron_ore', 'emerald_stone', 'ruby_stone', 'sapphire_stone', 'topaz_stone']
  ...
  async cookFish(): Promise<boolean> {
    for (const item of this.rawFishList) {
      ...
      if (numInBank == 0) {
        ...
        let itemToCraftSchema = await this.identifyCraftedItemFrom(item, 'cooking')
        if (!itemToCraftSchema) {
          break;
        }

        let numToCraft = Math.floor(numInBank / itemToCraftSchema.craft.items[0].quantity)
   ...
   */
  async identifyCraftedItemFrom(ingredient: string, craftSkill: CraftSkill): Promise<ItemSchema> {
    let craftedItemList = await getAllItemInformation({craft_material: ingredient, craft_skill: craftSkill, max_level: this.character.getCharacterLevel(craftSkill)})
    if (craftedItemList instanceof ApiError) {
      this.character.handleErrors(craftedItemList)
      return;
    } else if (craftedItemList.data.length === 0) {
      logger.info(`${craftSkill} isn't high enough to craft anything. Skipping`)
      return;
    }
    ...
    for (let craftItem of craftedItemList.data) {
      if (craftItem.craft.items.length > 1) {
        logger.debug(`${craftItem.code} requires more than 1 ingredient. Skipping`)
        continue;
      } else {
